  ...
  return (
    <button
      ...
      className={`w-full flex items-center gap-2 px-2 py-1.5 text-left text-sm rounded transition-colors ${
        selected ? 'bg-blue-100 dark:bg-blue-900 text-blue-900 dark:text-blue-100' : 'hover:bg-gray-100 dark:hover:bg-gray-700 text-text-secondary'
      }`}
      style={{ paddingLeft: `${level * 12 + 8}px` }}
    >
...
function renderTree(node, level, selectedKey, onSelectGroup, labelKey, envOrder) {
  if (node.groups) {
  ...
  return sortedKeys.map(key => (
    <TreeNode key={key} label={key.toUpperCase()} level={level} defaultExpanded>
      {renderTree(node[key], level + 1, selectedKey, onSelectGroup, labelKey, envOrder)}
    </TreeNode>
  ))
}
...
export default function TreeView({ targets, selectedGroup, onSelectGroup, envOrder = ['dev', 'nonprod', 'staging', 'demo', 'prod'] }) {
  ...
  const { tree, labelKey } = useMemo(() => buildTree(targets, groupBy), [targets, groupBy])
    ...
    <div className="h-full flex flex-col bg-surface border-r border-border">
      ...
      <div className="flex-1 overflow-y-auto p-2">
        {tree.groups ? (
          ...
          // Nested tree
          renderTree(tree, 0, selectedGroupKey, onSelectGroup, labelKey, envOrder)
        )}

        {targets.length === 0 && (
          <p className="text-sm text-text-muted p-2">No targets configured</p>
        )}
      </div>
    </div>
  )
