...
export const meta = {
  id: 'test',
  title: 'Test',
  description: 'Test variable interpolation and pipe functions with httpbin.org'
}
  ...
  return (
    <>
      <Section title="1. Basic GET Request">
        <Text>Test a simple GET request to httpbin.org. Save the origin IP for later use.</Text>
        <Request
          method="GET"
      ...
      <Section title="2. Variable Interpolation in URL">
        <Text>Use the saved IP in a URL parameter. Requires running step 1 first.</Text>
        <Request
          method="GET"
      ...
      <Section title="3. POST with Variable in Body">
        <Text>Send the IP in a POST body. Also demonstrates saving response data.</Text>
        <Request
          method="POST"
      ...
      <Section title="5. Pipe Functions - Array Functions">
        <Text>Test array pipe functions: first, last, sort, reverse, unique</Text>
        <Request
          method="GET"
      ...
      <Section title="11. Shell Command Execution">
        <Text>Test running local shell commands. The environment variables TARGET_ID, TARGET_NAME, etc. are available.</Text>
        <Request
          target="old"
          method="EXEC"
      ...
      <Section title="12. Saving Command Output">
        <Text>Demonstrates saving stdout to a variable, with optional trimming and parsing.</Text>
        <Request
          target="old"
          method="EXEC"
          path="date"
        ...
        />
        <Text>{"Verify saved variables: Date: {{currentDate}}, Status: {{statusValue}}"}</Text>
      </Section>

      <Section title="13. End-to-End Command Chain">
        <Text>{"Verify the full flow: execute -> save -> use in next command."}</Text>
        <Request
          target="old"
          method="EXEC"
